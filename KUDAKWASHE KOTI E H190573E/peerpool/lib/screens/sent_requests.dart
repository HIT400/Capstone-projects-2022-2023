...
import '../utils/on_refactor_time.dart';
...
class _SentRequestState extends State<SentRequest> {
  ...
  CountdownTimerController requestTimerController;
  int requestEndTime;
  ...
  onSetRequestTimer(request) async {
    var time = request['startDate'];

    var newdate = DateTime.parse(time);
    setState(() {
      requestEndTime = newdate.millisecondsSinceEpoch;
      requestTimerController =
          CountdownTimerController(endTime: requestEndTime);
    });
  }
          ...
          if (docs.isEmpty) {
            ...
            try {
              requestsAdded = docs.where((element) {
                ...
                List waitingList = opendata['waitingList'];
                var date = DateTime.now();
                return waitingList != null &&
                    waitingList
                            .where(
                                (e) => e['personEmail'] == user['emailAddress'])
                            .toList() !=
                        null;
              }).toList();
            } catch (e) {
  ...
  onResetRequests() async {
    setState(() {
      requestTimerController.dispose();
      requestEndTime = null;
      requestTimerController = null;
    });
  ...
  returnRequestItem(item, index) {
    ...
    if (DateTime.now().isBefore(DateTime.parse(item['startDate']))) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        onSetRequestTimer(item);
      });
    }
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: DateTime.now()
                    ...
                    : requestTimerController == null || requestEndTime == null
                        ? returnEmptyContainer()
                        : CountdownTimer(
                            endTime: requestEndTime,
                            controller: requestTimerController,
                            onEnd: onResetRequests,
                            endWidget: returnEmptyContainer(),
                            widgetBuilder: (_, CurrentRemainingTime time) {
                              ...
                              return Text(
                                "Your journey starts in " + refactorTime(time),
                                style: TextStyle(
                                  fontWeight: FontWeight.normal,
                                  fontSize: screenWidth * 0.032,
                                  fontFamily: lightfont,
